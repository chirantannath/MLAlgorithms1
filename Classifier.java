...
public interface Classifier<IR extends Row, OP> {
  ...
  /** @see #fit(IR, OP) */
  default void fit(Pair<? extends IR, ? extends OP> truePair) {
    fit(truePair.first(), truePair.second());
  }
  ...
  /**
   * Predict for an input. This NEEDS to be thread-safe (not change state of
   * model). Returns an optional value to permit failure to classify.
   */
  Optional<OP> predict(IR input);
   ...
   * @see #predict(IR)
   */
  default Stream<Optional<OP>> predict(Stream<? extends IR> inputs) {
    return inputs.map(this::predict);
  }
   ...
   */
  default long countCorrect(Iterator<? extends IR> inputs, Iterator<? extends OP> trueOutputs,
      LongConsumer testedCountConsumer) {
    ...
    while (inputs.hasNext()) {
      ...
      if (testedCountConsumer != null)
        testedCountConsumer.accept(++count);
      if (Objects.equals(output, predicted.orElse(null)))
        ++correctCount;
    }
    return correctCount;
  }
  ...
  default long countCorrect(Iterator<? extends Pair<? extends IR, ? extends OP>> truePairs,
      LongConsumer testedCountConsumer) {
    long count = 0, correctCount = 0;
    while (truePairs.hasNext()) {
      ...
      if (testedCountConsumer != null)
        testedCountConsumer.accept(++count);
      if (Objects.equals(pair.second(), predicted.orElse(null)))
        ++correctCount;
    }
    return correctCount;
  }
   ...
   * Put {@code testedCountConsumer = null} if no progress report is desired.
   */
  default long countCorrectParallel(Stream<? extends Pair<? extends IR, ? extends OP>> truePairs,
      LongConsumer testedCountConsumer) {
    final var count = new AtomicLong(0);
    var stream = truePairs.unordered().parallel();
    if (testedCountConsumer != null)
      stream = stream.peek(pair -> testedCountConsumer.accept(count.incrementAndGet()));
    return stream.filter(pair -> Objects.equals(pair.second(), predict(pair.first()).orElse(null))).count();
  }
}
